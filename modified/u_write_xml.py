		...
		for story_id in desc_ids: # iterate over all descriptions of a single chart
			...
			def build_approx_tags(parent_tag):
				...
				for i in range(1, len(approx_id_map)+1):					
					...
					if prev1 is not None: 
						prev1_label, prev1_start, prev1_end, prev1_text = prev1
						if prev1_label == "<y_axis_inferred_label>" and prev1_end+1 == curr_start:
							unit = prev1_text
			...
			def build_relex_tags(parent_tag):
				...
				for i in range(1, len(relex_id_map)+1):					
					...
					sorted_y_values = sorted(corpus['y'])  
					y_ordered_index = {
						"least":0, "Scnd":1, "3rd":2, "4th":3, "5th":4, "6th":5, "highest":len(sorted_y_values)-1
						}
					...
					add_relex.set("vg", str(vg))
					add_relex.set("va", str(va))
			...
			def build_hedges_tags(parent_tag):
				...
				for i in range(1, len(hedge_id_map)+1):		
					...
					hedged_expr = None
					if next1 in approx_id_map or next1 in relex_id_map:
						hedged_expr = next1
					elif next2 in approx_id_map or next2 in relex_id_map:
						hedged_expr = next2
					elif next3 in approx_id_map or next in relex_id_map:
						hedged_expr = next3
					
					# set hedge_type
					if hedged_expr is not None: 
						if hedged_expr in approx_id_map:
							hedge_type = "abs_ex"
