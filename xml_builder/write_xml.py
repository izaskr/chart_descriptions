...
import initialize_dicts 


descriptions_files, description_files_order, topic_image_id, descriptions_files_json = initialize_dicts.get_dicts("both")
...
def get_closest(num, l):
    ...
    """
    intvl = l[1]-l[0]
    # account for cases where the last major tick wasnt put in rudy's data
    l.append(l[-1]+intvl)
    if num in set(l):
        return str(num)+','+str(num)
    less = [i for i in l if i < num]
    greater = [i for i in l if i>num]
    return str(less[-1])+','+str(greater[0])
...
version_dir = "../data/" #assumes all description txt would be here, and the output also saved here  
...
def collect_parse(annotations, chart_id, calculated):
	""" 
	...
	version_path = version_dir + chart_id[:2] + '/' + chart_id + '/' + annotations

	#TODO: check encoding
	# with open(version_dir + annotations, "r", encoding="ISO-8859-1") as f:
	with open(version_path, "r", encoding="ISO-8859-1") as f:
		end_desc = False
...
def make_xml_tree():
	...
	for tri in zip(description_files_order, topic_image_id):
		...
		data = 'data/json_data/' + descriptions_files_json[annotations][0] + ".json"
		data_ext = get_x_y("../"+data)

		# for d, i in data_ext.items():
		# 	print(i, '\n\n')
		# exit()
		...
		if corpus["title"] != title:  
			exit('\nERROR: Make sure the title "' + annotations + '" exists in the file "' + descriptions_files_json[annotations][0] + '.json"\n')
		# print(corpus['title'], "       ", title)
		results = get_stat_info(corpus)
		for_xml = collect_parse(annotations, chart_id, results)
		...
		for story_id in desc_ids: # iterate over all descriptions of a single chart
			...
			def build_approx_tags(parent_tag):
				...
				for i in range(1, len(approx_id_map)+1):					
					...
					y_ordered_index = {
						"axis_highest":0 , "inferred_highest":0 , "Scnd":1, "3rd":2, "4th":3, "5th":4, "6th":5, "least":len(sorted_y_values)-1
						}


					for key in y_ordered_index:					
						#TODO: check this - changed aif statement to include not approximation-tags
						# if curr_label.find("inferred_"+key) != -1:
						if curr_label.find(key) != -1:
					...
					if va_index is not None: 
						...
						# print(va_index)
						# print(chart_id, corpus['title'], title, '\n\n')
					...
					if is_digit(va):
						...
						major_ts = corpus['y_axis_major_ticks']
						minor_ts = corpus['y_axis_minor_ticks']

						clst_major_ts = get_closest(va, major_ts)
						clst_minor_ts = get_closest(va, minor_ts)
						...
						minor_ti_dist = abs(minor_ts[1] - minor_ts[0])
						# print('\n\n', vg)
						# print(major_ts)
						# print(minor_ts)
					...
					c_major_ts = str(l_close_major_tick)+','+str(r_close_major_tick)
					add_approx.set("nearest_major_ticks", c_major_ts)


					c_minor_ts = str(l_close_minor_tick)+','+str(r_close_minor_tick)					
					add_approx.set("nearest_minor_ticks", c_minor_ts)

					add_approx.set('major_dist', str(major_ti_dist))
					add_approx.set('minor_dist', str(minor_ti_dist))
				...
				for i in range(1, len(relex_id_map)+1):					
					...
					for key in y_ordered_index:
						...
						if not v1 and curr_label.find("v1="+key) != -1:
							v1_key = key
							v1_sorted_index = y_ordered_index[key]
							v1 = sorted_y_values[v1_sorted_index]
							
						# set v2
						if not v2 and curr_label.find("v2="+key) != -1:
							v2_key = key
							v2_sorted_index = y_ordered_index[key]
					...
					if v1 and v2:
						if relex_type == "mul":
							...
							# assign vG + ##check
							# onl for digit-convertible
							if vg == "##REVIEW" and is_digit(curr_text):
								vg = float(curr_text)
								if not vg_sign:
									vg_sign = -1 if va < 0 else 1
								if  vg.is_integer():
									vg = int(vg)	
					...
					if va is not None and va !="##REVIEW" and str(abs(va)) == curr_text:
						vg = va



					global a_vg
					global a_vg_total, relex_va_to_review
					
					if va == "##REVIEW":
						relex_va_to_review += 1
						print(chart_id, quad, '#######################')
...
if __name__ == "__main__":
	...
	print(a_vg, a_vg_total)
	print('relex_va_to_review', relex_va_to_review)
