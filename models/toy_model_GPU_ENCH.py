...
parser.add_argument("-beam", required=False, help="size of the beam for beam search decoding", default=8, type=int)
...
def main(topicID):
    ...
    home_data_dir = "/home/CE/skrjanec/chart_descriptions/models/mt/"
    train_fname = "tatoeba.eng_cmn.train.tsv"
    val_fname = "tatoeba.eng_cmn.dev.tsv"
    train_dataset = reader.read(home_data_dir+train_fname)
    validation_dataset = reader.read(home_data_dir+val_fname)
    ...
    for i in range(n_epoch): # DONE make a variable
        ...
        print("BLEU VALIDATION", metrics["validation_BLEU"])
